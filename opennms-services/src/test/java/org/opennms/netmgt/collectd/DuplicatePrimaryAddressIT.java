...
public class DuplicatePrimaryAddressIT {
    ...
    public void testExisting() throws Exception {
        ...
        Mockito.verify(m_filterDao, times(1)).flushActiveIpAddressListCache();
        Mockito.verify(m_collectdConfigFactory, times(4)).getCollectdConfig();
        Mockito.verify(m_collectdConfigFactory, times(2)).interfaceInPackage(any(OnmsIpInterface.class), any(Package.class));
        Mockito.verify(m_collectdConfiguration, times(1)).getCollectors();
        Mockito.verify(m_collectdConfiguration, times(2)).getPackages();
        Mockito.verify(m_collectdConfiguration, times(1)).getThreads();
